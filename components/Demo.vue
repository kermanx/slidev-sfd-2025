...
<template>
  <div>
    ...
    <DemoSlide class="text-left">
      <div
        class="grid grid-cols-2 w-full h-full dark:bg-[#181819] transition-transform transform duration-500"
        style="width: 200%"
        :class="page === 1 ? '-translate-x-1/2' : ''"
      >
        <SlideContainer class="h-full">
          <component :is="getLayout(0)" v-bind="getAttrs(0)">
            <div v-html="getHTML(0)" />
          </component>
        </SlideContainer>
        <SlideContainer class="h-full" v-show="page === 1 || !isPrintMode">
